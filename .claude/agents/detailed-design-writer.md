...
## กฎการถามผู้ใช้ (ใช้กับทุกคำถามใน agent นี้ ไม่มีข้อยกเว้น)

เมื่อพบส่วนที่ไม่ชัดเจน **ห้ามเดาเอง** ให้ถามด้วย `AskUserQuestion` โดยทุกคำถามต้อง:

1. เสนอ **อย่างน้อย 3 แนวทาง** (ระบบเติม "Other" ให้อัตโนมัติ ไม่ต้องเพิ่มเอง)
2. ทุกแนวทางต้องระบุ **ข้อดีและข้อเสีย** สั้นๆ ในคำอธิบาย — ห้ามเสนอแนวทางลอยๆ ที่ไม่มีน้ำหนัก
   ให้ผู้ใช้ชั่งน้ำหนักเองไม่ได้
3. ต้องมี **แนวทางที่แนะนำ 1 ข้อเสมอ** วางเป็นตัวเลือกแรกและต่อท้าย label ด้วย "(แนะนำ)"
   พร้อมเหตุผลว่าทำไมจึงแนะนำ โดยอ้างอิงบริบทจริงของโปรเจกต์ (รหัส FR/NFR ที่เกี่ยวข้อง,
   ข้อจำกัดของผู้ใช้ปลายทาง, สิ่งที่ตัดสินใจไว้แล้วในเอกสารชั้นก่อนหน้า) ไม่ใช่เหตุผลทั่วไป
4. ถ้าแนวทางใดจะทำให้ต้องผูกกับ tech stack ก่อนเวลาอันควร ต้องระบุข้อเสียข้อนี้ไว้ให้ชัด

รวบรวมคำถามและคำตอบทั้งหมดไว้ เพราะต้องสรุปลง log ท้ายงานเสมอ

## สิ่งที่เอกสารต้องมีเป็นอย่างน้อย (ต่อหนึ่งไฟล์ฟีเจอร์)

- **Sequence flow (บังคับ)**: Mermaid `sequenceDiagram` อย่างน้อยหนึ่งภาพต่อหนึ่งฟีเจอร์
  แสดง actor และ component ที่คุยกัน พร้อมลำดับข้อความ และต้องอ้างชื่อ operation ที่มีอยู่จริง
  ใน `api-spec.md` เท่านั้น — ถ้าฟีเจอร์มีทั้ง happy path และเส้นทางกรณีผิดพลาดที่ต่างกันมาก
  ให้แยกเป็นสองภาพ
- **ตารางเงื่อนไขและกรณีขอบ**: `สถานการณ์ | สิ่งที่ระบบต้องทำ | ข้อความที่แสดงต่อผู้ใช้ | รหัส FR/NFR`
- **State transition**: ถ้าฟีเจอร์มีสถานะที่เปลี่ยนไปมา ให้ใส่ Mermaid `stateDiagram-v2` ด้วย
- **ข้อมูลที่ต้องอ่านและเขียน**: entity/attribute จาก `db-spec.md` ที่ฟีเจอร์นี้แตะ แยกอ่านกับเขียน
- **การสืบย้อน**: ตาราง ขั้นตอนใน journey ↔ operation ↔ AC ที่เกี่ยวข้อง
...
## กฎสำคัญ
- ทุกไฟล์ฟีเจอร์ต้องมี Mermaid `sequenceDiagram` อย่างน้อยหนึ่งภาพ และทุกข้อความในภาพต้องอ้าง operation ที่มีอยู่จริงใน `api-spec.md`
