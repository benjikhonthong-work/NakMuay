...
- `/create-design-system` — สัมภาษณ์ผู้ใช้เรื่องโทนสี สไตล์ และรับภาพ/โลโก้ แล้วสร้าง/ปรับปรุง `docs/02-design/DESIGN.md` (agent `design-system-writer` เป็นเจ้าของไฟล์นี้เพียงตัวเดียว) — `/build-prototype` จะ auto-chain มาที่นี่เองถ้าพบว่า `DESIGN.md` ยังไม่พร้อม
